...
let uid = router.currentRoute.value.query.uid as string || undefined;
let token = router.currentRoute.value.query.token as string || "token111";
...
onMounted(() => {
    ...
    // 获取IM的长连接地址
    APIClient.shared.post('https://yvmmss.top/api/getSocketAddress', {
        chat_id: router.currentRoute.value.query.chat_id,
...
// 连接IM
const connectIM = (addr: string, data: object) => {

    console.log('接口给过来的数据结果：', data)
    const config = WKSDK.shared().config

    // 设置当前用户的uid 和 token
    uid = data.chatInfo.uid
    token = data.chatInfo.token

    // 设置客服的uid
    channelID.value = data.customer.uid.toString()
    ...
    connectStatusListener = (status) => {
        if (status == ConnectStatus.Connected) {
            title.value = `${data.customer.name || ""}`
        } else {
            title.value = `${data.customer.name || ""}(断开)`
        }
    }
...
<template>
    <div class="chat">
        <div class="header">
            ...
            <div class="right">
                <button v-on:click="settingClick">
                    <!--                    {{-->
                    <!--                        to.channelID.length == 0 ? '与谁会话？' : `${to.channelType ==-->
                    <!--                        ChannelTypeGroup ? '群' : '单聊'}${to.channelID}`-->
                    <!--                    }}-->
                </button>
            </div>
        </div>
        ...
        <div class="footer">
            ...
            <button class="message-stream" v-on:click="onMessageStream">{{
                    startStreamMessage ? '停止流消息' : '开启流消息'
                }}
            </button>
